%% ver. 14a :: using lsqcurvefit
% if bexp (default), slower than FitAcr14 (taking 4x time)  :: because fitF includes external funciton FitAcr14exp (changed back to fminsearch)
% if ~bexp (fitF doesn't include external function), faster than FitAcr14 (taking 67% time)
% bme: inital ms and mf from me
% bexp: fit ms and mf only using FitAcr14exp
% bw: weighting radius and exponential?

function [Ms Mf D V A R GGf] = FitAcr14a(tau, GG, xzr, bme, bexp, bw, Tol)

ConvertMMA2;

if (nargin < 7) Tol = 1e-3;  end;
if (nargin < 6) bw = true;  end;
		...
		for iz=1:nz
			for ix=1:nx
				for iy=1:ny
					...
					% fit
					for ja=1:3
						if bexp		% use FitAcr14exp
                            %{
                            % lsqcurvefit is slower when fitF includes an external function
                            fitF = @(c,tn) c(1) + min(c(2),1-c(1)) .* FitAcr14exp(tn, g, c(1), min(c(2),1-c(1)), q,h,bw);
							fitC0 = [msi(ja) mfi(ja)];
                            fitCL = [0 0];  fitCU = [1 1];
							fitC = lsqcurvefit(fitF, fitC0, tn, g, fitCL, fitCU, optimset('Display','off','TolFun',Tol,'TolX',Tol));
							ms = fitC(1);  mf = min(fitC(2),1-ms);  
                            %}
                            ...
                            Vmax = pi/q/dt;  % q*Vz*dt = pi
                            Dmax = 100e-12;
							if xzr == 1
                                fitF = @(c,tn) c(1) + min(c(2),1-c(1)).*exp( -c(3).^2.*tn.^2 -c(4).*tn ).*exp(1i*q* sum(unwrap(angle(g-c(1))).*t)/sum(t.^2)/q *t);
								fitC0 = [msi(ja) mfi(ja) h*sqrt(vti(ja)^2+vzi(ja)^2)*tau(end) q^2*di(ja)*tau(end)];
                                fitCL = [0 0 0 0];  fitCU = [1 1 h*Vmax*tau(end) q^2*Dmax*tau(end)];
                            else
                                fitF = @(c,tn) c(1) + min(c(2),1-c(1)).*exp( -h^2*(sum(unwrap(angle(g-c(1))).*t)/sum(t.^2)/q)^2*t.^2 -c(3).^2.*tn.^2 -c(4).*tn ).*exp(1i*q* sum(unwrap(angle(g-c(1))).*t)/sum(t.^2)/q *t);
								fitC0 = [msi(ja) mfi(ja) hx*vti(ja)*tau(end) q^2*di(ja)*tau(end)];
                                fitCL = [0 0 0 0];  fitCU = [1 1 h*Vmax*tau(end) q^2*Dmax*tau(end)];
							end
							fitC = lsqcurvefit(fitF, fitC0, tn, g, fitCL, fitCU, optimset('Display','off','TolFun',Tol,'TolX',Tol));
							ms = fitC(1);  mf = min(fitC(2),1-ms);  d = fitC(4)/q^2/tau(end);
							vz = sum(unwrap(angle(g-ms)).*t)/sum(t.^2)/q;
							if xzr == 1
								v = max(fitC(3)/h/tau(end),abs(vz));  vt = sqrt(v^2-vz^2);
							else
								vt = fitC(3)/hx/tau(end);  v = sqrt(vt^2+vz^2);
							end
						end
